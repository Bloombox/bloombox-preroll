...
all: build
	@echo "bloombox-preroll is ready."
...
ifeq ($(BUILDBOT),yes)
test:
	wct --job-name "bloombox-preroll" --expanded --build-number "$(BUILD_NUMBER)" --sauce-tunnel-id "$(SAUCE_TUNNEL)" --plugin wct-jenkins
else
test:
	@wct --job-name "bloombox-preroll"
endif
